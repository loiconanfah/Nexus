...
```bash
node scripts/demo-drive-full.mjs          # démo locale
node scripts/demo-drive-full.mjs --deployed  # site en ligne
```

Les timecodes sont indicatifs (la latence IA de la scène 5 peut décaler la suite).
Le texte en gras est ce que tu dis.

---

### ~0:00 · Le problème (vitrine)
...
### ~0:53 · Simulation holographique — le moment clé
**« Et on peut le simuler en direct. On fait tomber un élément, et on regarde la panne se propager, en distinguant liens directs et indirects. »**
*(quand le panneau se remplit / on ouvre une carte)*
**« Chaque élément touché est détaillé : coût horaire, temps de rétablissement, probabilité. »**

### ~1:16 · Un impact logique, pas juste visuel
...
### ~2:29 · Risque & résilience
**« Côté résilience : les points uniques de défaillance, la concentration des fournisseurs, et les dépendances humaines — ces personnes clés dont dépendent des processus critiques. »**

### ~2:58 · La connaissance, augmentée
...
## Conseils
- Voix posée, phrases courtes ; laisse respirer sur la simulation (le « wow »).
- La scène 5 (inférence IA) attend un vrai calcul (~25 s) : profites-en pour poser le « moat ».
- Coupe le son système de Chrome. Version muette : sous-titres = les phrases en gras.
- Parcours reproductible : relance le script autant que nécessaire.
