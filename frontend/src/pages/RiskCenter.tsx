...
export function RiskCenter() {
  ...
  if (isLoading) return <div style={{ fontFamily: mono, color: 'var(--nx-text-muted)' }}>ASSESSING RISK…</div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 style={{ fontFamily: geist, fontSize: 24, letterSpacing: '-0.01em', color: 'var(--nx-text)' }}>Risk Intelligence</h2>
          <p className="mt-1" style={{ fontSize: 13, color: 'var(--nx-text-muted)' }}>Identify the dependencies most likely to disrupt critical operations.</p>
        </div>
        <div className="hidden items-center gap-2 rounded-sm border px-4 py-2 lg:flex" style={{ background: 'var(--nx-surface-container)', borderColor: 'var(--nx-border)', fontFamily: mono, fontSize: 12 }}>
          <span style={{ color: 'var(--nx-text-muted)' }}>System Status:</span>
          <span className="flex items-center gap-1" style={{ color: CYAN }}><span className="h-2 w-2 animate-pulse rounded-full" style={{ background: CYAN }} /> ONLINE</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        {TILES.map((t) => {
          const count = rows.filter((r) => t.bands.includes(r.band)).length
          const active = bandFilter && bandFilter.join() === t.bands.join()
          return (
            <button key={t.label} onClick={() => setBandFilter(active ? null : t.bands)}
              className="relative flex flex-col gap-1 overflow-hidden rounded-sm border p-3 text-left"
              style={{ background: 'var(--nx-surface-container)', borderColor: active ? t.color : 'var(--nx-border)' }}>
              <span style={{ fontFamily: mono, fontSize: 12, textTransform: 'uppercase', color: 'var(--nx-text-muted)' }}>{t.label}</span>
              <span style={{ fontFamily: geist, fontSize: 28, fontWeight: 500, color: t.color }}>{count}</span>
              <div className="absolute bottom-0 left-0 h-1 w-full" style={{ background: t.color, opacity: active ? 1 : 0.5 }} />
            </button>
          )
        })}
      </div>
  ...
  return (
    <div className="relative flex min-h-[380px] items-center justify-center rounded-sm border p-6" style={{ background: 'var(--nx-panel)', borderColor: 'var(--nx-border)' }}>
      ...
      <div className="relative flex aspect-square w-full max-w-[440px] flex-col">
        <div className="absolute -left-10 top-1/2 -translate-y-1/2 -rotate-90 whitespace-nowrap" style={{ fontFamily: mono, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--nx-text-muted)' }}>Business Impact</div>
        ...
        <div className="mt-3 text-center" style={{ fontFamily: mono, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--nx-text-muted)' }}>Likelihood</div>
      </div>
    </div>
  )
}
      ...
      <table className="w-full text-left">
        <thead>
          <tr className="border-b" style={{ borderColor: 'var(--nx-border)' }}>
            {['Risk', 'Asset', 'Business Impact', 'Propagation', 'Dependents', 'Score', 'Status'].map((h, i) => (
          ...
          {rows.map((r) => {
            ...
            return (
              <tr key={r.id} onClick={() => onSelect(r.id)} className="cursor-pointer border-b transition-colors"
                ...
                <td className="px-3 py-3" style={{ fontFamily: mono, fontSize: 12, color: 'var(--nx-text-muted)' }}>{r.entityType}</td>
                <td className="px-3 py-3"><span className="rounded px-2 py-0.5" style={{ fontFamily: mono, fontSize: 11, color: c, background: `color-mix(in srgb, ${c} 20%, transparent)`, border: `1px solid color-mix(in srgb, ${c} 30%, transparent)` }}>{r.band.toUpperCase()}</span></td>
                ...
                <td className="px-3 py-3" style={{ fontSize: 13, color: r.hasRedundancy ? 'var(--nx-text-muted)' : CYAN_T }}>{r.hasRedundancy ? 'Protected' : 'Open'}</td>
              </tr>
            )
          })}
        </tbody>
...
function PriorityRisk({ row, onClose, onSimulate, onView }: { row: RiskRow; onClose: () => void; onSimulate: () => void; onView: () => void }) {
  ...
  const bars = [
    { label: 'Criticality', v: factor('Criticality') },
    { label: 'Propagation', v: factor('PropagationPotential') },
    { label: 'Concentration', v: factor('Concentration') },
    { label: 'Redundancy', v: row.hasRedundancy ? 1 : 0.2 },
  ]
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-6 rounded-sm border p-5 lg:w-[320px]" style={{ background: 'var(--nx-surface)', borderColor: 'var(--nx-border)' }}>
      <div className="flex items-center justify-between">
        <h3 style={{ fontFamily: geist, fontSize: 18, fontWeight: 600, color: 'var(--nx-text)' }}>Priority Risk</h3>
      ...
      <div className="relative flex flex-col items-center justify-center overflow-hidden rounded-sm border p-6" style={{ background: 'var(--nx-surface-container)', borderColor: 'var(--nx-border)' }}>
        ...
        <span className="z-10" style={{ fontFamily: mono, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--nx-text-muted)' }}>Risk Score</span>
      ...
      <div className="flex flex-col gap-3">
        <h4 className="border-b pb-1" style={{ fontFamily: mono, fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--nx-text-muted)', borderColor: 'var(--nx-border)' }}>Detail Breakdown</h4>
        {bars.map((b) => {
        ...
        <div className="mt-2 flex items-center justify-between rounded-sm border p-2" style={{ background: 'var(--nx-surface-high)', borderColor: 'var(--nx-border)' }}>
          <span className="flex items-center gap-1" style={{ fontSize: 13, color: 'var(--nx-text)' }}><CheckCircle2 size={16} style={{ color: CYAN }} /> Confidence</span>
