...
const DOCS: readonly Doc[] = [
  {
    id: 'intro',
    icon: BookOpen,
    title: ['Qu’est-ce que Lenexux', 'What is Lenexux'],
    blocks: [
      { kind: 'p', text: [
        'Lenexux est une plateforme d’intelligence des dépendances opérationnelles. Elle modélise vos systèmes, applications, infrastructures, fournisseurs, personnes et processus sous forme d’un graphe de dépendances, puis analyse ce graphe pour révéler ce qui casse — avant que l’activité n’en souffre.',
        'Lenexux is an operational dependency intelligence platform. It models your systems, applications, infrastructure, suppliers, people and processes as a dependency graph, then analyzes that graph to reveal what breaks — before the business feels it.',
      ] },
      { kind: 'p', text: [
        'La devise résume la promesse : « Savoir ce qui casse avant que l’activité n’en pâtisse ». Concrètement, Lenexux répond à trois questions : où sont mes points uniques de défaillance ? que se passe-t-il si tel élément tombe ? combien ça coûte et combien de temps pour récupérer ?',
        'The tagline sums up the promise: “Know what breaks before the business does.” In practice, Lenexux answers three questions: where are my single points of failure? what happens if a given element fails? how much does it cost and how long to recover?',
      ] },
  ...
  },
  {
    ...
    blocks: [
      { kind: 'p', text: [
        'Tout dans Lenexux est soit une entité (un nœud), soit une relation (une arête orientée entre deux entités). C’est ce qui permet de suivre une dépendance de bout en bout.',
        'Everything in Lenexux is either an entity (a node) or a relation (a directed edge between two entities). That is what makes it possible to trace a dependency end to end.',
      ] },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'p', text: [
        'Un point unique de défaillance (SPOF) est un élément dont beaucoup dépendent sans alternative : sa panne se propage largement. Lenexux les détecte structurellement et les met en tête.',
        'A single point of failure (SPOF) is an element that many depend on with no alternative: its failure propagates widely. Lenexux detects them structurally and surfaces them first.',
      ] },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'p', text: [
        'À l’import, Lenexux reconnaît les synonymes et variantes de types (ex. « depends on », « DependsOn » → DEPENDS_ON) au lieu de rejeter la ligne. Les cas inconnus retombent sur des valeurs sûres.',
        'On import, Lenexux recognizes synonyms and type variants (e.g. “depends on”, “DependsOn” → DEPENDS_ON) instead of dropping the row. Unknown cases fall back to safe defaults.',
      ] },
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      { kind: 'p', text: [
        'L’Assistant IA répond en langage naturel, mais les chiffres viennent toujours des moteurs déterministes de Lenexux. L’IA ne fait que formuler la réponse — elle n’invente aucune donnée.',
        'The AI Analyst answers in natural language, but the numbers always come from Lenexux’s deterministic engines. The AI only phrases the answer — it invents no data.',
      ] },
      { kind: 'ul', items: [
  ...
  },
  {
    ...
    blocks: [
      { kind: 'ul', items: [
        ...
        ['SSO Entra ID (Microsoft) disponible : connexion avec le compte professionnel, sans mot de passe Lenexux (activé par l’administrateur du déploiement).', 'Entra ID (Microsoft) SSO available: sign in with the corporate account, no Lenexux password (enabled by the deployment admin).'],
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'p', text: [
        'Non. Lenexux fonctionne par import : vous n’exposez que ce que vous choisissez de partager. Les connexions directes en lecture seule sont une étape ultérieure et optionnelle.',
        'No. Lenexux works by import: you only expose what you choose to share. Direct read-only connections are a later, optional step.',
      ] },
      ...
      { kind: 'p', text: [
        'Écrivez à support@nexus.io. Pour un déploiement (SSO, clés, connexions directes), contactez votre administrateur Lenexux.',
        'Email support@nexus.io. For a deployment (SSO, keys, direct connections), contact your Lenexux administrator.',
      ] },
    ],
  },
] as const
